...
def UIResults(freq, n):
    ...
    try:
        ...
        IGain.config(text=f'{Gain:.3f} dbi')

    except ValueError:
        messagebox.showerror(title='Error Message', message='Invalid Input!\nPlease Enter a Valid Number!', )
...
def reset():
    ...
    # Reset Outputs
    CoilD.config(text='-- mm')
    CoilS.config(text='-- mm')
    CoilH.config(text='-- mm')
    CoilAlpha.config(text='-- degrees')
    Axial.config(text='--')
    IGain.config(text='-- dbi')
...
CoilAlpha = ttk.Label(root, width=15, text='-- degrees')
...
Axial = ttk.Label(root, width=10, text='--', justify='left')
...
IGain = ttk.Label(root, width=15, text='-- dbi')
